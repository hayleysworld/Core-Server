...
var commands = exports.commands = {
	...
	readfile: function(target, room, user, connection){
		...
		if(!target) return this.sendReply('/readfile chatrooms or /readfile ranks');
		var self = this;
		switch(target){
			case 'chatrooms':
				fs.readFile('./config/chatrooms.json', function(err, data){
					if(err) throw err;
					self.sendReply('|popup|' + data);
				});
				break;
			case 'ranks':
				...
				break;
			default:
				return this.sendReply('Valid targets are chatrooms or ranks.');
		}
	},

	baeoftheday: 'botd',
	botd: function (target, room, user) {
		if (room.id !== 'baeroom') return this.sendReply("This command can only be used in Bae Room!.");
		if (!this.canBroadcast() || !room.chatRoomData) return;
		if (!target) {
			if (!room.chatRoomData.bae) return this.sendReplyBox("The Bae of the Day has not been set.");
			return this.sendReplyBox("The current <strong>' Bae of the Day'</strong> is:<br />" + room.chatRoomData.bae);
		}
		if (!this.can('declare', null, room)) return false;
		if (target === 'off' || target === 'disable' || target === 'reset') {
			if (!room.chatRoomData.bae) return this.sendReply("The Bae of the Day has already been reset.");
			delete room.chatRoomData.quote;
			this.sendReply("The Bae of the Day was reset by " + Tools.escapeHTML(user.name) + ".");
			this.logModCommand(user.name + " has reset the Bae of the Day.");
			Rooms.global.writeChatRoomData();
			return;
		}
		room.chatRoomData.bae = Tools.escapeHTML(target);
		room.addRaw(
			'<div class="broadcast-green">' +
				"<p><strong>The 'Bae of the Day' has been updated by " + Tools.escapeHTML(user.name) + ".</strong></p>" +
				"<p>Quote: " + room.chatRoomData.bae + '</p>' +
			'</div>'
		);
		this.logModCommand(Tools.escapeHTML(user.name) + " has updated the bae of the day to: " + room.chatRoomData.bae);
		Rooms.global.writeChatRoomData();
	},
	...
	popup: function(target, room, user){
		...
		targetUser.sendReply('|popup|' + user.name + ' says: ' + target);
	},
	...
	afk: function(target, room, user, connection) {
		...
		if (!user.isAfk) {
			...
			if(target){
				this.add('|html|<b>'+user.realName+'</b> is now away ('+target+').');
			} else {
				this.add('|html|<b>'+user.realName+'</b> is now away.');
			}
			...
			this.sendReply('You are already AFK. Running /unafk');
			return this.parse('/unafk');
			}
		user.updateIdentity();
	},
	...
	unafk: function(target, room, user, connection) {
		if (!user.isAfk) {
			...
			this.add('|html|<b>' + newName + '</b> is back in business');
			user.isAfk = false;
	...
	hide: 'hideauth',
        hideauth: function(target, room, user){
                ...
                if(target){
                        target = target.trim();
                        if((Config.groupsranking.indexOf(target) > -1) && (target !== 'locked') && (target !== 'muted')){
	...
	image: function(target, room, user){
		...
		this.sendReply('|raw|<img src="' + Tools.escapeHTML(target) + '">');
	},
